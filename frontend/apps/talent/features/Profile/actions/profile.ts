...
export const showProfile = async (): Promise<Profile> => {
  try {
    const talentToken = `Bearer ${(await cookies()).get("talentToken")?.value}`;
    const res = await getProfile({ headers: { "Talent-Authorization": talentToken } });

    switch (res.status) {
      case 200:
        return res.data;
      default:
        throw new Error("Failed to show profile", { cause: res.data });
    }
  } catch (error) {
...
export const updateProfile = async (data: ProfileUpdateInput): Promise<ProfileUpdateResponse> => {
  try {
    const talentToken = `Bearer ${(await cookies()).get("talentToken")?.value}`;
    const res = await putProfile(data, { headers: { "Talent-Authorization": talentToken } });
