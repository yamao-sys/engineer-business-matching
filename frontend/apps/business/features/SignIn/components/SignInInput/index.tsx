...
const SignInInput: FC = () => {
  const [validationError, setValidationError] = useState<CompanySignInResponse["error"]>("");

  const { control, handleSubmit } = useForm<CompanySignInInput>();
  const router = useRouter();

  const onMutate = useCallback(() => setValidationError(""), [setValidationError]);
  const onMutateSuccess = useCallback(
    (result: CompanySignInResponse) => {
      if (result.error) {
